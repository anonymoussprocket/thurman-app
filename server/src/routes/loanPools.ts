...
import { createPoolFromCSV, approveAndDeploy, retryPoolCreation, retryPoolConfiguration, retryLoanDeployment } from '../services/loanPool';
import { generateFilePreview } from '../services/csvProcessor';
import { findUserPools, findPendingPools, findActivePools, rejectLoanPool } from '../prisma/models';
import { createUploadMiddleware, extractFormData } from '../services/upload';
import { handleMulterError, validateUploadedFile } from '../middleware/upload';
import { getDeployedPools, getPoolById, getCacheStatus, PoolData, PoolDataResponse } from '../services/poolData';
...
loanPoolsRouter.post('/', requireAuth, uploadLoanDataFile, handleMulterError, async (req: AuthRequest, res: express.Response, next: NextFunction) => {
    try {
...
loanPoolsRouter.patch('/:id/approve', requireAuth, requireRole([UserRole.ADMIN]), async (req: AuthRequest, res: express.Response, next: NextFunction) => {
    try {
...
loanPoolsRouter.patch('/:id/reject', requireAuth, requireRole([UserRole.ADMIN]), async (req: AuthRequest, res: express.Response, next: NextFunction) => {
    try {
        const adminId = req.user?.id;
        if (!adminId) {
            return res.status(401).json({
...
loanPoolsRouter.get('/admin/pending-approvals', requireAuth, requireRole([UserRole.ADMIN]), async (req: AuthRequest, res: express.Response, next: NextFunction) => {
    try {
...
loanPoolsRouter.post('/preview', uploadLoanDataFile, handleMulterError, async (req: express.Request, res: express.Response, next: NextFunction) => {
    try {
        const file = req.file;
...
loanPoolsRouter.post('/:id/retry-step', requireAuth, requireRole([UserRole.ADMIN]), async (req: AuthRequest, res: express.Response, next: NextFunction) => {
    try {
        const adminId = req.user?.id;
        const poolId = parseInt(req.params.id);
        const { step } = req.body;
