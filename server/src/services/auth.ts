...
export const createToken = (email: string, userId: number, walletId: string | null | undefined, address: string | undefined): Secret => {
    return sign({ email, userId, walletId, address }, config.jwtSecretKey, { expiresIn: '1d' });
};

export const signup = async ({ email, password, accountType, blockchains, walletSetId }: SignupParams): Promise<SignupReturnParams | null> => {
    try {
...
export const login = async ({ email, password }: LoginParams): Promise<LoginReturnParams | null> => {
    if (!email || !password) {
    ...
    try {
        ...
        const token = createToken(user.email, user.id, user.wallets[0].id, user.wallets[0].address);
