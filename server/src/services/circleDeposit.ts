...
import { parseUSDCAmount, formatPoolId, formatAddress } from './utils';
import { validateWalletPermissions, getUserWalletId, getAdminWalletId } from './walletService';
 ...
 */
export const createUSDCApproval = async (amount: string, userWalletId: string): Promise<TransactionResponse> => {
    try {
 ...
 * @returns Transaction response with ID
 */
export const createDepositRequest = async (poolId: number, amount: string, userAddress: string, userWalletId: string): Promise<TransactionResponse> => {
    try {
        ...
        const parameters = [formatPoolId(poolId), parseUSDCAmount(amount), formatAddress(userAddress)];
 ...
 * @returns Transaction response with ID
 */
export const createDepositFulfillment = async (poolId: number, amount: string, userAddress: string, adminWalletId: string): Promise<TransactionResponse> => {
    try {
 ...
 * @returns Transaction response with ID
 */
export const createShareClaim = async (poolId: number, amount: string, userAddress: string, userWalletId: string): Promise<TransactionResponse> => {
    try {
