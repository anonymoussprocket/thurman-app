 ...
 * );
 */
export function usePolling<T>(endpoint: string, options: PollingOptions<T> = {}): PollingResult<T> {
    const { interval = 5000, enabled = true, requiresAuth = false, requiresAdmin = false, onDataChange, onError, onNotification, retryAttempts = 3, maxBackoffInterval = 30000 } = options;
...
export function useUserPolling<T>(endpoint: string, options: Omit<PollingOptions<T>, 'requiresAuth'> = {}): PollingResult<T> {
    return usePolling<T>(endpoint, { ...options, requiresAuth: true });
}

export function useAdminPolling<T>(endpoint: string, options: Omit<PollingOptions<T>, 'requiresAdmin'> = {}): PollingResult<T> {
    return usePolling<T>(endpoint, { ...options, requiresAdmin: true });
}

export function useFrequentPolling<T>(endpoint: string, options: Omit<PollingOptions<T>, 'interval'> = {}): PollingResult<T> {
    return usePolling<T>(endpoint, { ...options, interval: 3000 });
}

export function useSlowPolling<T>(endpoint: string, options: Omit<PollingOptions<T>, 'interval'> = {}): PollingResult<T> {
