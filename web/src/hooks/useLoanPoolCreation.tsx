  ...
  const submitLoanPool = useCallback(async () => {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        const errorMessage = error.response?.data?.message || error.response?.data?.error || error.message || 'Failed to create loan pool';
        return {
          success: false,
          error: errorMessage
        };
...
function getStepFields(step: StepType): (keyof LoanPoolData)[] {
  switch (step) {
    case 'metadata':
      return ['name', 'description', 'targetAmount', 'minimumInvestment', 'expectedReturn', 'maturityDate', 'purpose', 'geographicFocus', 'borrowerProfile', 'collateralType', 'loanTermRange', 'interestRateRange'];
    case 'file':
      return ['loanDataFile'];
    case 'review':
