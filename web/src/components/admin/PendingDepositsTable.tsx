...
import { Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Button, Typography, Box, Chip, Skeleton, Alert, Dialog, DialogTitle, DialogContent, DialogActions, IconButton, Tooltip, Stack, Card, CardContent, Divider, Grid } from '@mui/material';
import { CheckCircle as CheckCircleIcon, ContentCopy as CopyIcon, Refresh as RefreshIcon, Warning as WarningIcon, Schedule as ScheduleIcon, AccountBalance as AccountBalanceIcon } from '@mui/icons-material';
...
const PendingDepositsTable: React.FC<PendingDepositsTableProps> = ({ onDataChange }) => {
          ...
          if (timeSinceLastNotification > 10000) {
            ...
            notificationTimeoutRef.current = setTimeout(() => {
              ...
              newDeposits.forEach((deposit: PendingDeposit) => {
                enqueueSnackbar(`New deposit request: ${formatCurrency(deposit.amount)} from ${formatAddress(deposit.userAddress)}`, {
                  variant: 'info',
  ...
  const sortedDeposits = useMemo(() => {
    return [...deposits].sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
  }, [deposits]);
    ...
    async (deposit: PendingDeposit) => {
      ...
      try {
        ...
        if (response.data.success) {
          enqueueSnackbar(`Fulfilled ${formatCurrency(deposit.amount)} deposit for ${formatAddress(deposit.userAddress)}`, {
            variant: 'success',
    ...
    <Box>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
        ...
        <Tooltip title='Refresh'>
          <IconButton onClick={refetch} disabled={loading} sx={{ color: 'primary.main' }}>
            <RefreshIcon />
          </IconButton>
        </Tooltip>
      </Box>
              ...
              {sortedDeposits.map(deposit => {
                ...
                return (
                  <TableRow key={deposit.id} hover>
                    <TableCell>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        ...
                        <Tooltip title='Copy address'>
                          <IconButton size='small' onClick={() => handleCopyAddress(deposit.userAddress)} sx={{ p: 0.5 }}>
                    ...
                    <TableCell>
                      <Chip label={deposit.poolName || `Pool ${deposit.poolId}`} size='small' variant='outlined' />
                    </TableCell>
                    <TableCell>
      ...
      {/* Confirmation Dialog */}
      <Dialog open={confirmDialog.open} onClose={() => setConfirmDialog({ open: false, deposit: null })} maxWidth='sm' fullWidth>
        <DialogTitle sx={{ fontWeight: 600 }}>Confirm Deposit Fulfillment</DialogTitle>
        <DialogContent>
          {confirmDialog.deposit && (
            <Stack spacing={2}>
              <Typography variant='body1'>Are you sure you want to fulfill this deposit request?</Typography>
        ...
        <DialogActions>
          <Button onClick={() => setConfirmDialog({ open: false, deposit: null })} disabled={fulfillingDeposits.has(confirmDialog.deposit?.id || '')}>
            Cancel
          </Button>
          <Button onClick={handleConfirmFulfill} variant='contained' color='success' disabled={fulfillingDeposits.has(confirmDialog.deposit?.id || '')} startIcon={<CheckCircleIcon />}>
