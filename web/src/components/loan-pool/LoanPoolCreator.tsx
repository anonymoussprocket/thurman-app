...
export default function LoanPoolCreator() {
  ...
  const { formMethods, currentStep, currentStepIndex, nextStep, previousStep, goToStep, fileState, handleFileAccepted, handleFileRejected, handleRemoveFile, submitLoanPool, resetForm } = useLoanPoolCreation();
  ...
  const renderCurrentStep = () => {
    ...
    switch (currentStep) {
      ...
      case 'review':
        return <ReviewStep {...stepProps} onSubmit={handleSubmit} goToStep={goToStep} />;
      default:
        return <BasicInfoStep {...stepProps} />;
    }
  };
    ...
    <BackgroundContainer>
      <ContentContainer>
        <SimpleFormContainer>
          ...
          {/* Step Indicator */}
          <StepIndicator currentStep={currentStepIndex} steps={STEP_LABELS} />

          {/* Error Display */}
          {submitError && (
            <Box sx={{ mb: 3 }}>
              <Alert severity='error' onClose={() => setSubmitError(null)} sx={{ borderRadius: '1.25em' }}>
                {submitError}
              </Alert>
            </Box>
          )}
