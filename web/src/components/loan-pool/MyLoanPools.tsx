...
// External libraries
import { Box, Table, TableContainer, TableHead, TableBody, TableRow, TableCell, Button, Chip, Typography, Skeleton, Paper, Alert, Container, Card, CardContent, Grid, useTheme, useMediaQuery } from '@mui/material';
  ...
  const fetchPools = useCallback(async () => {
    try {
      ...
      if (axios.isAxiosError(err)) {
        const errorMessage = err.response?.data?.message || err.response?.data?.error || err.message || 'Failed to fetch loan pools';
...
const ErrorState = ({ error, onRetry }: { error: string; onRetry: () => void }) => (
  <Box sx={styles.containers.backgroundContainer}>
    <Container maxWidth='lg' sx={styles.containers.content}>
      ...
      <Button variant='contained' onClick={onRetry} sx={styles.button.primary}>
        Try Again
      </Button>
    </Container>
  </Box>
    ...
    <Container maxWidth='lg' sx={styles.containers.content}>
      ...
      <Paper sx={{ p: 4, textAlign: 'center', ...styles.containers.formContainer }}>
        ...
        <Typography variant='body1' sx={{ mb: 3, color: 'text.secondary' }}>
          Loan pools allow you to bundle multiple loans together for investors. Create your first pool to get started.
        </Typography>

        <Button variant='contained' startIcon={<Add />} onClick={onCreatePool} sx={styles.button.primary}>
...
// Table row component
const PoolTableRow = ({ pool, onViewDetails }: { pool: LoanPool; onViewDetails: (id: number) => void }) => {
  ...
  return (
    <TableRow sx={styles.table.row}>
        ...
        <Box>
          ...
          {pool.description && (
            <Typography variant='body2' color='text.secondary' sx={{ mt: 0.5 }}>
              {pool.description.length > 50 ? `${pool.description.substring(0, 50)}...` : pool.description}
            </Typography>
          )}
        </Box>
      </TableCell>
      <TableCell sx={styles.table.cell}>
        <Chip label={statusConfig.text} color={statusConfig.color as any} size='small' variant='outlined' sx={styles.table.statusChip} />
      </TableCell>
      <TableCell sx={styles.table.cell}>
        <Typography variant='body2'>{pool.total_loans.toLocaleString()}</Typography>
      </TableCell>
      <TableCell sx={styles.table.cell}>
      ...
      <TableCell sx={styles.table.cell}>
        <Typography variant='body2'>{formatDate(pool.created_at)}</Typography>
      </TableCell>
      <TableCell sx={styles.table.cell}>
        <Button
          variant='outlined'
...
const PoolCard = ({ pool, onViewDetails }: { pool: LoanPool; onViewDetails: (id: number) => void }) => {
  const statusConfig = getStatusConfig(pool.status);
          ...
          <Box sx={{ flex: 1 }}>
            ...
            {pool.description && (
              <Typography variant='body2' color='text.secondary' sx={{ mb: 1 }}>
                {pool.description.length > 80 ? `${pool.description.substring(0, 80)}...` : pool.description}
              </Typography>
            )}
          </Box>
          <Chip label={statusConfig.text} color={statusConfig.color as any} size='small' variant='outlined' sx={styles.table.statusChip} />
    ...
    <Box sx={styles.containers.backgroundContainer}>
      <Container maxWidth='lg' sx={styles.containers.content}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
          ...
          <Button variant='contained' startIcon={<Add />} onClick={handleCreatePool} sx={styles.button.primary}>
            Create Pool
          </Button>
        </Box>
          ...
          <Grid container spacing={2}>
            {pools.map(pool => (
              <Grid item xs={12} key={pool.id}>
                <PoolCard pool={pool} onViewDetails={handleViewDetails} />
              </Grid>
            ))}
          </Grid>
        ) : (
          ...
          <Paper sx={styles.table.container}>
            <TableContainer sx={{ maxHeight: 600 }}>
              <Table stickyHeader>
                ...
                <TableBody>
                  {pools.map(pool => (
                    <PoolTableRow key={pool.id} pool={pool} onViewDetails={handleViewDetails} />
                  ))}
                </TableBody>
              </Table>
            </TableContainer>
