...
export default function DepositInterface({ poolId, poolName, minDeposit = 0.01, maxDeposit = 1000000, hideBalanceCard = false }: DepositInterfaceProps) {
  ...
  const { data: depositStatus, loading: depositLoading, error: depositError, refetch: refetchDepositStatus } = usePolling<DepositStatus>(depositStatusEndpoint, pollingOptions);
  ...
  return (
    <Box>
      {/* Global Notification */}
      <Snackbar open={!!notification} autoHideDuration={6000} onClose={clearNotification} anchorOrigin={{ vertical: 'top', horizontal: 'center' }}>
      ...
      <Grid container spacing={3}>
        ...
        {!hideBalanceCard && (
          <Grid item xs={12} md={4}>
            <UserBalanceCard userAddress={userAddress} onBalanceUpdate={handleBalanceUpdate} />
          </Grid>
        )}

        {/* Deposit Status Card - Adjust width based on balance card visibility */}
        <Grid item xs={12} md={hideBalanceCard ? 12 : 8}>
          <DepositStatusCard depositStatus={depositStatus} loading={depositLoading} poolId={poolId} poolName={poolName} onClaimSuccess={handleClaimSuccess} onClaimError={handleClaimError} />
        </Grid>

        {/* Deposit Section - Only show for authenticated users */}
        {user && (
          <Grid item xs={12}>
            <DepositSection poolId={poolId} poolName={poolName} minDeposit={minDeposit} maxDeposit={maxDeposit} userBalance={userBalance} onDepositSuccess={handleDepositSuccess} onDepositError={handleDepositError} />
          </Grid>
        )}
        ...
        {!user && (
          <Grid item xs={12}>
            <Alert severity='info' sx={{ borderRadius: '1.25em' }}>
              ...
              <Typography variant='body2'>Please sign in to your account to make deposits and track your investment status.</Typography>
            </Alert>
          </Grid>
        )}
      </Grid>
