...
import { TrendingUp as TrendingUpIcon, Schedule as ScheduleIcon, AccountBalance as AccountBalanceIcon } from '@mui/icons-material';
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ mb: 4 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
          <Chip label={`${pool.expected_return}% APY`} color='primary' icon={<TrendingUpIcon />} sx={{ borderRadius: '1em' }} />
          <Chip label={`Min: $${pool.minimum_investment.toLocaleString()}`} variant='outlined' icon={<AccountBalanceIcon />} sx={{ borderRadius: '1em' }} />
          <Chip label={`Matures: ${pool.maturity_date.toLocaleDateString()}`} variant='outlined' icon={<ScheduleIcon />} sx={{ borderRadius: '1em' }} />
        </Box>
      </Box>
        ...
        <Grid item xs={12} md={3}>
          ...
          >
            <CardContent>
              ...
              <Chip label={pool.status} color={pool.status === 'DEPLOYED' ? 'success' : 'warning'} sx={{ borderRadius: '1em', fontWeight: 600 }} />
            </CardContent>
          </Card>
        </Grid>
      </Grid>
      ...
      <Box>
        ...
        <DepositInterface poolId={pool.id} poolName={pool.name} minDeposit={parseFloat(pool.config.minDepositAmount)} maxDeposit={parseFloat(pool.config.maxDepositAmount)} />
      </Box>
    </Box>
  );
}
