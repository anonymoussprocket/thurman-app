...
export const SUPPORTED_FILE_TYPES = {
    CSV: ['.csv'],
    EXCEL: ['.xlsx', '.xls'],
    MIME_TYPES: ['text/csv', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'application/vnd.ms-excel']
} as const;
...
export function validateFileType(file: File): boolean {
    ...
    const hasValidExtension = [...SUPPORTED_FILE_TYPES.CSV, ...SUPPORTED_FILE_TYPES.EXCEL].some(ext => fileName.endsWith(ext));
...
export async function parseFilePreview(file: File): Promise<LoanFilePreview> {
    try {
        ...
        if (axios.isAxiosError(error)) {
            const errorMessage = error.response?.data?.message || error.response?.data?.error || error.message || 'Failed to process file preview';
