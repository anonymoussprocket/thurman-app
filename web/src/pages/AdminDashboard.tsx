...
import { Box, Typography, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Chip, Avatar, IconButton, Tooltip } from '@mui/material';
          ...
          >
            <TableContainer>
              <Table>
                <TableHead>
                  <TableRow>
                    <TableCell sx={{ fontWeight: 600, color: '#29262a' }}>User</TableCell>
                    <TableCell sx={{ fontWeight: 600, color: '#29262a' }}>Role</TableCell>
                    ...
                    <TableRow key={user.id}>
                      <TableCell>
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                          <Avatar sx={styles.avatar.small}>{user.email.charAt(0).toUpperCase()}</Avatar>
                          <Typography sx={{ ml: 2, color: '#29262a' }}>{user.email}</Typography>
                        </Box>
                      </TableCell>
                      <TableCell>
                        <Chip label={user.role} color={getRoleColor(user.role)} size='small' sx={{ borderRadius: '0.25em' }} />
                      </TableCell>
                      <TableCell>
                        <Chip label={user.status} color={getStatusColor(user.status)} size='small' sx={{ borderRadius: '0.25em' }} />
                      </TableCell>
                      <TableCell sx={{ color: '#29262a' }}>{user.createdAt}</TableCell>
                      <TableCell>
                        <Box sx={{ display: 'flex', gap: 1 }}>
                          <Tooltip title='View Details'>
                            <IconButton size='small' sx={{ color: '#725aa2' }}>
                              <VisibilityIcon />
                            </IconButton>
                          </Tooltip>
