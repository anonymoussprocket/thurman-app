...
import { Box, Typography, Grid, Card, CardContent, Chip, Skeleton, Button, Alert, Divider, Paper, Stack, IconButton, Tooltip } from '@mui/material';
import { ArrowBack as ArrowBackIcon, TrendingUp as TrendingUpIcon, AccountBalance as AccountBalanceIcon, Schedule as ScheduleIcon, Refresh as RefreshIcon, CheckCircle as CheckCircleIcon } from '@mui/icons-material';
      ...
      <CardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <Grid container spacing={3} sx={{ flexGrow: 1 }}>
          ...
          <Grid item xs={12} sm={6}>
            ...
            <Chip label={pool.config?.depositsEnabled ? 'Open for Deposits' : 'Closed'} color={pool.config?.depositsEnabled ? 'success' : 'default'} size='small' variant={pool.config?.depositsEnabled ? 'filled' : 'outlined'} />
          </Grid>
        </Grid>
        ...
        <Box sx={{ py: 4 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 4 }}>
            ...
            <Tooltip title='Refresh'>
              <IconButton onClick={() => window.location.reload()} sx={{ color: 'primary.main' }}>
                <RefreshIcon />
              </IconButton>
            </Tooltip>
          </Box>
          ...
          {pool && (
            <Box sx={{ mb: 4, display: 'flex', gap: 2, flexWrap: 'wrap' }}>
              <Chip label={pool.config?.depositsEnabled ? 'Open for Deposits' : 'Closed'} color={pool.config?.depositsEnabled ? 'success' : 'default'} variant={pool.config?.depositsEnabled ? 'filled' : 'outlined'} />
              <Chip label={`Pool ID: ${pool.id}`} variant='outlined' size='small' />
            </Box>
          )}
          ...
          <Grid container spacing={3}>
            ...
            {pool?.config?.depositsEnabled && (
              <Grid item xs={12} lg={4} sx={{ display: 'flex' }}>
                <UserBalanceCard userAddress={user?.account} onBalanceUpdate={() => {}} />
              </Grid>
            )}
